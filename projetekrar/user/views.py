...
def userRegister(request):
    if request.method=='POST':
        ...
        dogum_tarih = request.POST['dogum-tarih']
        cinsiyet = request.POST['cinsiyet']
        
        if User.objects.filter(email=email).exists():
            messages.error(request,'mail hatalı')
            ...
            musteri=Musteri.objects.create(
                user=user , isim=isim , soyisim=soyisim , email=email , dogum_tarihi=dogum_tarih, cinsiyet=cinsiyet , cep_telefonu=telefon
...
def userLogout(request):
    logout(request)
    messages.error(request,'cıkış Yapıldı')
    return redirect('giris-yap')
...
def hesabimBilgiler(request):
    if request.method=='POST':
        ...
        musteri.save()
        user.save()
        messages.success(request,'Kullanıcı bilgileriniz basarıyla güncellendi')
...
def sifreDegistir(request):
    if request.method=='POST':
        ...
        if user.check_password(mevcut_sifre):
            ...
            messages.error(request,'Mevcut şifre yanlış veya şifreler aynı değil')
            return redirect('hesabim-sifre')
